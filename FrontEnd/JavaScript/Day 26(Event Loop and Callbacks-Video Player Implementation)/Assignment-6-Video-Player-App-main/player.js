...
for (let i = 0; i < playlistData.length; i++) {
  playlistwrapper.innerHTML += `
    <div id="card${playlistData[i].id}" class="playlist-card ${
    i == 0 ? "active-card" : ""
  }" onclick="renderVideo(${playlistData[i].id})">
...
// For render the cards
// function renderVideo(id){
//     alert(`Hey! You just clicked the card with id of: ${id}`)
// }
...
function renderVideo(id) {
  ...
  playerwrapper.innerHTML = `
        <iframe id="video-player" src="https://player.vimeo.com/video/${
          data.vimeoId
        }" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>

                    <div>
                        <div id="video-actions">
                            <p><span id="views-count">${
                              data.views / 1000
                            }k</span> views</p>
